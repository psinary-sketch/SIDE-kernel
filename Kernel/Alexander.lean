...
theorem delta_at_one : 1 ^ 2 - 1 + 1 = (1 : Int) := by decide

-- Delta(-1) = 3 (knot determinant = second generating prime)
theorem delta_at_neg_one : (-1) ^ 2 - (-1) + 1 = (3 : Int) := by decide

-- 6 = 2 * 3 (cyclotomic index = product of generating primes)
theorem six_eq : 2 * 3 = (6 : Nat) := by decide

-- The discriminant of t^2 - t + 1 is -3
-- disc = b^2 - 4ac = 1 - 4 = -3
-- This is the Trivium field Q(sqrt(-3))
theorem discriminant : 1 ^ 2 - 4 * 1 * 1 = (-3 : Int) := by decide
...
theorem genus : (2 - 1) * (3 - 1) / 2 = (1 : Nat) := by decide
