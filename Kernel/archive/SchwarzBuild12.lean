...
theorem schwarz_discharge (s : ℂ) :
  ...
  suffices hpt : EqOn
      (fun t => (↑t : ℂ) ^ ((starRingEnd ℂ) s / 2 - 1) • (hurwitzEvenFEPair 0).f_modif t)
      (fun t => (starRingEnd ℂ) ((↑t : ℂ) ^ (s / 2 - 1) • (hurwitzEvenFEPair 0).f_modif t))
      (Ioi 0) by
  ...
  congr 1
  · -- cpow part: rewrite exponent then apply cpow_conj
