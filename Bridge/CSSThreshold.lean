...
theorem css_fails_dim2 : 2^(2-2) % 2 = 1 := by decide

-- Self-orthogonality HOLDS at dimension 3
theorem css_holds_dim3 : 2^(3-2) % 2 = 0 := by decide

-- Diagonal entries (2^(n-1)) are even for n ≥ 2
theorem diagonal_even_dim2 : 2^(2-1) % 2 = 0 := by decide
theorem diagonal_even_dim3 : 2^(3-1) % 2 = 0 := by decide
...
theorem steane_n : 2^3 - 1 = 7 := by decide
theorem steane_rank : 2 * 3 = 6 := by decide
theorem steane_k : 7 - 2 * 3 = 1 := by decide
theorem steane_d : 2 + 1 = 3 := by decide
...
theorem stormer_235_not_mersenne : 10 ≠ 2^4 - 1 := by decide
